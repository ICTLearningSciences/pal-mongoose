...
GoalCohort.statics.joinOrCreateCohort = async function(user, goal, opts) {
  ...
  const cohort = userCohort ? userCohort.cohort : null;

  let goalCohort = await goalCohortModel.findUserCohort(user, goal);
  if (goalCohort) {
    // Already in the cohort
    if (isSameCohortName(goalCohort.cohort, cohort)) {
      return goalCohort;
    }
...
GoalCohort.statics.joinCohort = async function(user, goal, code) {
  ...
  if (curCohort) {
    ...
    if (
      ...
      !isSameCohortName(curCohort.cohort, cohort)
    ) {
...
const isSameCohortName = (a, b) => {
  const nameA = a ? a.nameCanonical : null;
  const nameB = b ? b.nameCanonical : null;
  return nameA === nameB;
};
