...
/**
 * Finds the GoalCohort with the given name
 */
GoalCohort.statics.findCohortByName = async function(name, goal) {
  const goalCohortModel = this;
...
GoalCohort.statics.joinOrCreateCohort = async function(user, goal, opts) {
  ...
  const userCohortName = userCohort ? userCohort.cohort : undefined;

  let cohort = await goalCohortModel.findUserCohort(user, goal);
  if (cohort) {
    if (cohort.nameCanonical === canonicalize(userCohortName)) {
      return cohort;
    }
  ...
  if (userCohort) {
    cohort = await goalCohortModel.findCohortByName(userCohortName, goal);
    if (cohort && cohort.memberSlotsRemaining === 0) {
      const err = new Error(
        `All teams in cohort ${userCohortName} are full. Please create a new team to join.`
      );
      err.status = 409;
      throw err;
    }
  ...
  cohort = await goalCohortModel.findOneAndUpdate(
    {
      goal: goal.id,
      nameCanonical: canonicalize(userCohortName),
  ...
  if (cohort.memberSlotsRemaining === -1) {
...
GoalCohort.statics.joinCohort = async function(user, goal, code) {
  ...
  let cohort = await goalCohortModel.findOne({
    goal: goal.id,
    nameCanonical: userCohort ? canonicalize(userCohort.cohort) : undefined,
    teams: {
      $elemMatch: {
        inviteCode: code
      }
  ...
  if (cohort.memberSlotsRemaining === 0) {
    const err = new Error(`The team cannot accept any more members`);
    err.status = 409;
    throw err;
  }
  ...
  if (curCohort) {
    ...
    // if user is in a different cohort, leave the current cohort
    if (
      userCohort &&
      curCohort.nameCanonical !== canonicalize(userCohort.cohort)
    ) {
      await goalCohortModel.leaveCohort(user, goal);
    }
  }
...
GoalCohort.statics.createTeam = async function(user, goal, teamname) {
  ...
  cohort = await goalCohortModel.findOne({
    ...
    memberSlotsRemaining: { $gt: userCohort ? -1 : 0 }
  });
  ...
  if (cohort) {
    if (cohort.teams.find(x => x.name === teamname)) {
      throw createError(
        409,
    ...
    teamIcon = DEFAULT_COHORT_TEAMS[cohort.teams.length % 6].icon;
  }
