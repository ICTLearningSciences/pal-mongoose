...
const compareCohorts = (cohort1, cohort2) => {
  if (!cohort1 && !cohort2) {
    return true;
  }
...
GoalCohort.statics.joinOrCreate = async function(user, goal, opts) {
  ...
  // We're in the case of having a newly created cohort; repair memberSlotsRemaining
...
GoalCohort.statics.joinWithInvite = async function(user, goal, code) {
  ...
  let goalCohort = goalCohorts.find(g => compareCohorts(g.cohort, cohort));
  ...
  goalCohort = goalCohorts.find(
    g => compareCohorts(g.cohort, cohort) && g.memberSlotsRemaining > 0
  );
