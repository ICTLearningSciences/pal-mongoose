...
GoalCohort.statics.generateInviteCode = async function() {
  const goalCohortModel = this;
  let code = shortid.generate();

  while (
    await goalCohortModel.exists({
      teams: {
        $elemMatch: {
          inviteCode: code
        }
...
GoalCohort.statics.joinOrCreate = async function(user, goal, opts) {
  ...
  // repair memberSlotsRemaining and give each team an invite code
...
GoalCohort.statics.createTeam = async function(user, goal, teamname) {
  ...
  const teamCode = await goalCohortModel.generateInviteCode();
...
GoalCohort.statics.invite = async function(user, goal) {
  ...
  const changesAsSet = {};
  for (let i = 0; i < cohort.teams.length; i++) {
    const team = cohort.teams[i];
    if (!team.inviteCode) {
      changesAsSet[
        `teams.${i}.inviteCode`
      ] = await goalCohortModel.generateInviteCode();
    }
  }
