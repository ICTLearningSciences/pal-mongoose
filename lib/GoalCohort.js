...
GoalCohort.statics.joinOrCreateCohort = async function(user, goal, opts) {
  ...
  userCohort = userCohort ? userCohort.cohort : null;

  let cohort = await goalCohortModel.findUserCohort(user, goal);
  if (cohort) {
    if (cohort.cohort === userCohort) {
      return cohort;
    }
  ...
  if (userCohort) {
    cohort = await goalCohortModel.findOne({ cohort: userCohort, goal: goal });
    ...
    if (cohort && cohort.memberSlotsRemaining === 0) {
      ...
      const name = names
        ...
        .join("");
      return await goalCohortModel.createTeam(user, goal, name);
    }
  }
...
GoalCohort.statics.joinCohort = async function(user, goal, code) {
  ...
  if (curCohort) {
    ...
    if (_team.inviteCode !== code || curCohort.cohort !== userCohort) {
