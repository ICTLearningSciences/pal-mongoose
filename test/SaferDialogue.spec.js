...
const SaferDialogue = require("SaferDialogue");

describe("SaferDialogue", function() {
  beforeEach(async () => {
  ...
  describe("findOneByIdOrAlias", function() {
    it("finds one by alias", async () => {
      const item = await SaferDialogue.findOneByIdOrAlias(
        "Goal_SuicidePrevention_Node_Root"
      );
      expect(item).to.have.property(
        "intro",
        "Pick a common concern, or ask your own question:"
      );
      expect(item).to.have.property(
        "question",
        "What kind of help do you need?"
      );
      expect(item.speech).to.eql(["Hello! What kind of help do you need?"]);
      expect(item).to.have.property("type", "Link");
      expect(item.nodes).to.eql([
      ...
      expect(item).to.have.property("content", null);
    });
  });

  describe("paginate", function() {
    it("finds an initial page of items with a default limit", async () => {
      const results = await SaferDialogue.paginate(
        {},
        {
          sort: { alias: 1 }
        }
      ...
      expect(results.items.length).to.eql(10);
